...
const char *volumeFilename = "voxel";
...
int main(int argc, const char** argv) 
{
    ...
    oclCheckErrorEX(cPathAndName != NULL, shrTRUE, pCleanup);
    size_t size = volumeSize[0] * volumeSize[1] * volumeSize[2] * CHANNEL_NUM;
...
void initCLVolume(uchar *h_volume)
{
    ...
    if (g_bImageSupport) {
        ...
        volume_format.image_channel_order = CL_ARGB;
        volume_format.image_channel_data_type = CL_UNSIGNED_INT8;
        ...
        d_volumeArray = clCreateImage3D(cxGPUContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, &volume_format,
                                        ...
                                        volumeSize[0]*CHANNEL_NUM,volumeSize[0] * volumeSize[1]*CHANNEL_NUM,
                                        h_volume, &ciErrNum);

        ciErrNum |= clSetKernelArg(ckKernel, 6, sizeof(cl_mem), (void *) &d_volumeArray);
        oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
    }

    // init invViewMatrix
    d_invViewMatrix = clCreateBuffer(cxGPUContext, CL_MEM_READ_ONLY, 12 * sizeof(float), 0, &ciErrNum);
    ciErrNum |= clSetKernelArg(ckKernel, 5, sizeof(cl_mem), (void *) &d_invViewMatrix);
    oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
}
